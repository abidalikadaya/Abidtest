var app = angular.module('employeeApp', ['ui.router', 'app.routes', 'app.config', 'ngAnimate', 'ngTouch', 'ui.grid', 'ui.bootstrap', 'angucomplete-alt', 'ngStorage', 'angular-growl', 'ngFileUpload','ui.grid.selection', 'angular-loading-bar','ui.grid.pagination', 'd3', 'd3tip']);

app.run(['$rootScope', '$state', '$localStorage', '$http', 'server', 'apis', 'growl', '$timeout', function ($rootScope, $state, $localStorage, $http, server, apis, growl, $timeout) {
	//$(document).ajaxStart(function() { Pace.restart(); });

	$rootScope.pluginRoles = $localStorage.pluginRoles;
	$rootScope.hotelRoles = $localStorage.hotelRoles;

	$rootScope.$state = $state;

	// Custom application logic start
	...
	//$(document).ready(function () {
		...
		// Toggling Sidebars
		// ------------------------------
		$('html').off('click', '#trigger-sidebar>a').on('click','#trigger-sidebar>a', function () {
			Utility.toggle_leftbar();
		});

		$('html').off('click', '#trigger-fullscreen').on('click','#trigger-fullscreen', function () {
			Utility.toggle_fullscreen(document.documentElement);
		});

		//Extrabar
		$('html').off('click', '#trigger-extrabar>a').on('click','#trigger-extrabar>a', function () {
			$('body').toggleClass('extrabar-show')
		});

		$('html').off('click', '.extrabar-underlay').on('click','.extrabar-underlay', function (event) {
		...
		// ------------------------------
		$('body').off('click', '.yamm .dropdown-menu, .dropdown-menu-form').on('click', '.yamm .dropdown-menu, .dropdown-menu-form', function (e) {
			e.stopPropagation()
		})
		...
		// -------------------------------
		$(document).off('click', '.dropdown-menu a[data-toggle="tab"]').on('click','.dropdown-menu a[data-toggle="tab"]', function (e) {
			e.stopPropagation();
			$(this).tab('show');
		...
		// Back to Top button
		// -------------------------------
		$('html').off('click', '#back-to-top').on('click','#back-to-top', function () {
			$('body,html').animate({
				scrollTop: 0
			}, 500);
			return false;
		...
		// Panel Collapses
		// -------------------------------
		$('html').off('click', 'a.panel-collapse').on('click','a.panel-collapse', function () {
		...
		// Search on Top
		// -------------------------------
		$('html').off('click', '#trigger-toolbar-search').on('click','#trigger-toolbar-search', function () {
			$("#toolbar-search").toggleClass('active');
		});

		$('html').off('click', '#toolbar-search .remove').on('click','#toolbar-search .remove', function () {
			$("#toolbar-search").toggleClass('active');
		});
	...
	// Custom application logic end

	$http.get(server.baseApiUrl + apis.checkAuth).then(function (res) {
		if (!!res.data.error) {
			$localStorage.$reset();
			$state.go('login');
		}
	...
	$rootScope.$on('$stateChangeStart', function (e, toState, toParams, fromState, fromParams) {
		...
		if (toState.name.indexOf('login') === -1 && !$localStorage.loggedIn) { // Go to landing page, if not logged in
			e.preventDefault();
			$state.go('login');
		} else if (toState.name.indexOf('login') !== -1 && !!$localStorage.loggedIn) { //Going to home page, if already logged in
			e.preventDefault();
			$state.go('home');
		}
	});
...
app.directive('numberOnlyInput', function () {
    return {
        restrict: 'EA',
        template: '<input name="{{inputName}}" class="form-control" ng-disabled="inputDisable" ng-change="inputChange()" ng-model="inputValue" />',
        scope: {
            inputValue: '=',
			inputDisable: '=',
			inputChange: '&',
...
app.config(['growlProvider', 'cfpLoadingBarProvider', function (growlProvider, cfpLoadingBarProvider) {
	growlProvider.globalTimeToLive(5000);
	growlProvider.globalPosition('bottom-right');
	cfpLoadingBarProvider.spinnerTemplate = '<div id="overlay-screen"></div>';
}]);