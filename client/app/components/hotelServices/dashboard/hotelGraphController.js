(function (app) {
    ...
    (function (hotelGraph) {
        ...
        var hotelGraphController = function ($scope, $uibModal, $log, $http, server, apis, growl, $localStorage, d3, d3tip) {
            ...
            $http.get(server.baseApiUrl+apis.getHotelDashProjectWithPeriod).then(function(projectListWithYear){
                if(projectListWithYear.data.success){                    
                    ...
                    $scope.projectPeriod = $scope.projectWithYearMap[$scope.selectedProject][0].periodYear;
                    $scope.selectedPeriod = new Date().getFullYear();
                    $scope.getdashboardData($scope.selectedProject,$scope.selectedPeriod);
                }else{
            ...
            $scope.groupBarChart = function (data, elementId) {
                angular.element(elementId).empty();
                var margin = {
                        top: 20,
                        right: 20,
                        bottom: 30,
                ...
                var color = d3.scale.ordinal()
                    .range(["#A5CCFF", "#FF9391", "#B0DB5C", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);
            ...
            $scope.pieChart = function (data, elementId) {
                ...
                var color = d3.scale.ordinal()
                    .range(["#05BC57", "#FF8A25", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);
         ...
         $scope.getdashboardData = function(selectedProject, selectedPeriod){
            //Get Revenue Data
            $http.post(server.baseApiUrl + apis.getDashboardRevenueData, { "projectId" : selectedProject, "projectYear" : selectedPeriod}).then(function (res) {
            ...
            //Get occupancy Data
            $http.post(server.baseApiUrl + apis.getDashboardOccupanyData, { "projectId" : selectedProject, "projectYear" : selectedPeriod}).then(function (res) {
            ...
            $http.post(server.baseApiUrl + apis.getDashboardScopeRevData, { "projectId" : selectedProject, "projectYear" : selectedPeriod}).then(function (res) {
