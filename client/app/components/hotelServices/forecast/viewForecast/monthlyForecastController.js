(function (app) {
    ...
    (function (viewForecast) {
        ...
        var hotelViewForecastController = function ($scope, $rootScope, $uibModal, $log, $http, server, apis, growl, $localStorage, $timeout) {
            //$scope.budgetViewText = true;
           // $scope.budgetViewInput = false;
            $scope.cloneForecastData = [];
            ...
            $scope.updateCalculation = function(daysCount){
                var revenueRow = 15,
                    accumulativeRow = 16;
                _.each( $scope.forecastData , function(val, key){
                    _.each( $scope.daysInMonthList , function(val1, key1){
                        var monthwiseData = _.map($scope.forecastData, parseInt(val1));
                        $scope.forecastData[4][val1] = Math.round((parseInt(monthwiseData[1]) / parseInt(monthwiseData[0])) * 100);
                        $scope.forecastData[5][val1] = Math.round(((parseInt(monthwiseData[11]) / parseInt(monthwiseData[3])) * 100) - 100);
                        $scope.forecastData[6][val1] = Math.round(parseInt(monthwiseData[12]) / parseInt(monthwiseData[1]));
                        $scope.forecastData[7][val1] = Math.round((parseInt(monthwiseData[8]) * parseInt(monthwiseData[9])) + parseInt(monthwiseData[12])) / parseInt(monthwiseData[1]).toFixed(2);
                        $scope.forecastData[10][val1] = Math.round(parseInt(monthwiseData[12]) / parseInt(monthwiseData[0]));
                        $scope.forecastData[15][val1] = Number((Number(monthwiseData[12]) + Number(monthwiseData[13]) + Number(monthwiseData[14])).toFixed(2));
                    });
                });
                ...
                _.each( $scope.daysInMonthList , function(val1, key1){
                    ...
                    var sumData = _.at($scope.forecastData[revenueRow], monthsTemp),
                        sumRow = _.sum(sumData);
                    $scope.forecastData[accumulativeRow][val1] = sumRow.toFixed(2);
                });
            }

            $scope.updateForecastData = function(selectedProject, selectedPeriod, selectedMonth){
                $scope.daysInMonthList = [];
                ...
                $http.post(server.baseApiUrl+apis.getHotelForecast, {projectId : selectedProject, projectYear:selectedPeriod }).then(function(response){
                    ...
                    $scope.cloneForecastData = _.clone($scope.forecastData);
                    var monthwiseData = _.map($scope.forecastData, selectedMonthName),
                        forecastDivideData = "";
                    _.each( $scope.forecastData , function(val, key){
                        forecastDivideData = (_.includes(fieldDividerArr, key)) ? (monthwiseData[key] / daysCount).toFixed(2) : '' ;
                        val.total = (accumulativeRow !== key) ? monthwiseData[key] : "";
                        _.each( $scope.daysInMonthList , function(val1, key1){
                            val[val1] = forecastDivideData;
                        });
                    });
        ...
        hotelViewForecastController.$inject = ['$scope','$rootScope','$uibModal','$log', '$http', 'server', 'apis', 'growl', '$localStorage', '$timeout'];
        
        angular.module('employeeApp').controller('app.hotelViewForecastController', hotelViewForecastController);
